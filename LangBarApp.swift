...
func mergeParagraphBlocks(_ bs: [Block]) -> [Block] {
    ...
    // 对称判断：两块是否属于同一段落（不依赖数组顺序）。
    //   归一化坐标原点在左下：minY 越大越靠屏幕上方，minY 较大者视为 upper。
    //   多栏杂志布局下，必须要求“水平方向显著重叠”才能合并，否则不同栏但 Y 相近的行会跨栏误并。
    func canMerge(_ a: Block, _ b: Block) -> Bool {
        let upper = a.box.minY >= b.box.minY ? a : b
        let lower = a.box.minY >= b.box.minY ? b : a
        ...
        if vGap < -0.5 * lineH { return false }        // 重叠过多/顺序异常 → 不合并
        let ox = min(upper.box.maxX, lower.box.maxX) - max(upper.box.minX, lower.box.minX)
        let minW = max(0.0001, min(upper.box.width, lower.box.width))
        let overlapRatio = ox / minW                   // 水平重叠占较窄块宽度比例
        return overlapRatio >= 0.5                      // 同栏（≥50% 重叠）才合并，杜绝跨栏
    }
    // 分离不可合并块（und/name/num/zh 原样保留）
    var result: [Block] = []
    var pool: [Block] = []
    for b in bs {
    ...
    // 并查集：任意两块同语种且 canMerge 成立即 union（对称，摆脱“数组相邻=图像相邻”假设）
    ...
    // 提速：按 minY 降序，仅在竖直邻近窗口内两两判断，利用 vGap 早停避免 O(n^2) 全配对。
    //   能合并的两块 vGap <= 1.5*lineH，故一旦候选块 maxY 低于当前块 minY 超过 1.5*maxLineH，
    //   后续块（更低）不可能再与当前块合并，直接 break——不漏任何合法配对，结果与全配对等价。
    let maxLineH = pool.map { $0.box.height }.max() ?? 0
    let order = Array(0..<n).sorted { pool[$0].box.minY > pool[$1].box.minY }
    for a in 0..<n {
        let i = order[a]
        let iMinY = pool[i].box.minY
        for b in (a + 1)..<n {
            let j = order[b]
            if pool[j].box.maxY < iMinY - 1.5 * maxLineH { break }
            if pool[i].lang == pool[j].lang && canMerge(pool[i], pool[j]) {
                union(i, j)
            }
        }
    }
    ...
    for (_, idxs) in groups {
        if idxs.count == 1 {
            result.append(pool[idxs[0]])
        } else {
            // 组内按阅读顺序拼接：minY 降序、同高 minX 升序
            let ordered = idxs.sorted { p, q in
                if abs(pool[p].box.minY - pool[q].box.minY) > 0.005 { return pool[p].box.minY > pool[q].box.minY }
                return pool[p].box.minX < pool[q].box.minX
            }
            let minX = ordered.map { pool[$0].box.minX }.min()!
            let minY = ordered.map { pool[$0].box.minY }.min()!
            let maxX = ordered.map { pool[$0].box.maxX }.max()!
            let maxY = ordered.map { pool[$0].box.maxY }.max()!
            let text = ordered.map { pool[$0].text }.joined(separator: "\n")
            result.append(Block(text: text,
                                box: CGRect(x: minX, y: minY, width: maxX - minX, height: maxY - minY),
                                lang: pool[ordered[0]].lang))
        }
    }
    return result
}
...
func annotate(_ cg: CGImage, blocks: [Block], breakdown: [(String, Int)], mixed: Bool,
    ...
    // 已回退段落合并（准确率优先）：恢复逐行标注。
    //   合并在封面/海报等大字号+多栏+含图形的布局下会跨图形过度合并成巨框，且无法用几何阈值稳健区分，
    //   而合并仅为视觉美化、不提升识别准确率，故回退到逐行标注最忠实。mergeParagraphBlocks 保留但不再调用。
