#!/usr/bin/env bash
# =============================================================================
# setup_fasttext.sh — 在 Mac 上一键准备 fastText CLI 二进制 + lid.176.bin 模型
#
# 用途：LangBarApp 的「任务B / fastText 补充验证层」默认走 Process() 调用本地
#       fasttext CLI（方式2，最简单、免改 Xcode 工程）。本脚本负责：
#         1) 安装/编译 fasttext 可执行文件
#         2) 下载语种识别模型 lid.176.bin（完整版约 126MB，Facebook 官方，176 语言，精度更高）
#         3) 放到 App 能自动发现的位置
#
...
#   模型:   $FASTTEXT_MODEL → App.app 内 Resources/lid.176.bin
#           → ~/lang-detect-mac/Resources/lid.176.bin → ./Resources/lid.176.bin
# =============================================================================
set -euo pipefail
...
MODEL_PATH="$RES_DIR/lid.176.bin"
...
echo ""
echo "提示：若想让打包后的 .app 自带模型，请在 build_app.sh 里把 Resources/lid.176.bin"
